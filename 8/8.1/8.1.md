...
<table border="1" cellpadding="5" cellspacing="0">
    ...
    <tr>
        ...
        <td>
            ...
            <img src="../8.1/Images_module2/registro_cat_2.JPG" alt="cat_2" >
...
<1>, <2>, <3>, <4>, <5> son ingresados por el usuario. <2>, <3>, <4> y <5> se seleccionan con listas desplegables.   
...
```sql
select concat(e.primer_nombre, e.primer_apellido, e.segundo_apellido) AS "Nombre del supervisor" from empleado e
inner join revision_calidad rc on rc.cod_supervisor = e.codigo_empleado 
where rc.cod_ordencompra = 18
limit 1
```
...
descripcion = '<2>'
FROM Orden_comprainsumo oc2
JOIN calidad c ON c.estadp = '<1>'
WHERE 
    oc2.cod_ordencompra = rc.cod_ordencompra
    AND rc.cod_insumo = oc2.cod_insumo
    AND oc2.cod_ordencompra = '<3>'
    AND oc2.Cod_Insumo = <4>;
```
...
```sql
select i.cod_insumo, i.nombre_insumo, oc2.cantidad_compra, rc2.cantidad_revisada, c.estado , rc.descripcion  from revision_calidad rc 
inner join revision_cantidad rc2 on rc.cod_ordencompra = rc2.cod_ordencompra 
inner join insumo i on rc.cod_insumo = i.cod_insumo 
inner join orden_compra oc on oc.cod_ordencompra = rc.cod_ordencompra
left join orden_comprainsumo oc2 on oc.cod_ordencompra = oc2.cod_ordencompra 
inner join calidad c on c.cod_calidad = rc.cod_calidad 
where rc.cod_ordencompra = <1>
```
...
![alt text](../8.1/Imagenes_modulo5/IngresarInicial.png)

```sql
select ci.nombre_categoriainsumo from insumo i
...
```sql
select i.cod_insumo, i.nombre_insumo, um.nombre_unidad, s.cantidad,ta.nombre_tipo_almacen, a.cod_almacen from insumo i 
inner join unidad_medidad um on um.cod_unidad = i.cod_unidad 
...
```sql
select ci.nombre_categoriainsumo, s.nombre_subcategoria, i.nombre_insumo,
...
ta.nombre_tipo_almacen , a.cod_almacen, l.nombre_local , p.nombre_empresa , s.cod_stock , 
