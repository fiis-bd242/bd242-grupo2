...
-- Crear la función index_dp()
CREATE OR REPLACE FUNCTION index_dp()
RETURNS NUMERIC AS $$
DECLARE
    index_dp_max NUMERIC;
BEGIN
    SELECT MAX(CAST(REGEXP_REPLACE(cod_dp, '[^0-9]', '', 'g') AS NUMERIC)) INTO index_dp_max
    FROM DETALLE_PEDIDO;

    index_dp_max := index_dp_max + 1;

    RETURN index_dp_max;
END;
$$ LANGUAGE plpgsql;

-- Crear la función set_cod_dp() para el trigger
CREATE OR REPLACE FUNCTION set_cod_dp()
RETURNS TRIGGER AS $$
BEGIN
    NEW.cod_dp := 'DP' || index_dp();  -- Asignar el nuevo código DP llamando a la función index_dp
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Crear el trigger que se ejecuta antes de insertar en DETALLE_PEDIDO
CREATE TRIGGER trigger_set_cod_dp
BEFORE INSERT ON DETALLE_PEDIDO
FOR EACH ROW
EXECUTE FUNCTION set_cod_dp();


-- ITEM DETALE PEDIDO
CREATE OR REPLACE FUNCTION index_item()
RETURNS NUMERIC AS $$
DECLARE
    index_item_max NUMERIC;
BEGIN
    SELECT MAX(CAST(REGEXP_REPLACE(cod_item_dp, '[^0-9]', '', 'g') AS NUMERIC)) INTO index_item_max
    FROM ITEM_DETALLE_PEDIDO;

	index_item_max := index_item_max +1;
	
    RETURN index_item_max;  -- Retorna el valor máximo
END;
$$ LANGUAGE plpgsql;

-- Crear Trigger que asigna el código automaticamente
CREATE OR REPLACE FUNCTION set_cod_item()
RETURNS TRIGGER AS $$
BEGIN
    NEW.cod_item_dp := 'IDP' || index_item();  -- Concatenar 'DP' con el índice calculado
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
-- Crear el trigger que se ejecuta antes de insertar en ITEM_DETALLE_PEDIDO
CREATE TRIGGER trigger_set_cod_item
BEFORE INSERT ON ITEM_DETALLE_PEDIDO
FOR EACH ROW
EXECUTE FUNCTION set_cod_item();
